...
namespace Wob_CharGen {
	[BepInPlugin( "Wob.CharGen", "Character Generation Mod", "1.2.0" )]
	public partial class CharGen : BaseUnityPlugin {
		...
		protected void Awake() {
			...
			WobSettings.Add( new WobSettings.Entry[] {
				...
				new WobSettings.Boolean(         "Traits",   "HideContrarian",          "Remove the Contrarian trait from heirs, but still apply its effects if locked", false                            ),
			} );
		...
		// Replace random trait generation method call with a modified one
		...
		internal static class CharacterCreator_GenerateRandomCharacter_Patch {
			internal static IEnumerable<CodeInstruction> Transpiler( IEnumerable<CodeInstruction> instructions ) {
				WobPlugin.Log( "CharacterCreator.GenerateRandomCharacter Transpiler Patch" );
				// Set up the transpiler handler with the instruction list
				WobTranspiler transpiler = new WobTranspiler( instructions );
				// Perform the patching
				...
				transpiler.PatchAll(
						// Define the IL code instructions that should be matched
						new List<WobTranspiler.OpTest> {
							// GenerateClass(classType, characterData);
							// Vector2Int randomTraits = GetRandomTraits(forceRandomizeKit);
							/*  0 */ new WobTranspiler.OpTest( OpCodes.Ldfld, name: "classType"      ), // classType
							/*  1 */ new WobTranspiler.OpTest( OpCodes.Ldloc_1                       ), // characterData
							/*  2 */ new WobTranspiler.OpTest( OpCodes.Call, name: "GenerateClass"   ), // GenerateClass(classType, characterData)
							/*  3 */ new WobTranspiler.OpTest( OpCodes.Ldarg_1                       ), // forceRandomizeKit
							/*  4 */ new WobTranspiler.OpTest( OpCodes.Call, name: "GetRandomTraits" ), // GetRandomTraits(forceRandomizeKit)
						},
						// Define the actions to take when an occurrence is found
						new List<WobTranspiler.OpAction> {
							new WobTranspiler.OpAction_SetOperand( 4, SymbolExtensions.GetMethodInfo( () => NewGetRandomTraits( false, null ) ) ), // Call replacement GetRandomTraits method
							new WobTranspiler.OpAction_Insert( 4, OpCodes.Ldloc_1 ),
						} );
				// Return the modified instructions
				return transpiler.GetResult();
			}

			private static Vector2Int NewGetRandomTraits( bool forceRandomizeKit, CharacterData characterData ) {
				...
				if( forceRandomizeKit || WobSettings.Get( "SoulShop", "AllSlotsLock_Contrarian", false ) || traitTypes[0] == TraitType.RandomizeKit || traitTypes[1] == TraitType.RandomizeKit ) {
					// Check if the contarian trait should be hidden
					if( WobSettings.Get( "Traits", "HideContrarian", false ) ) {
						...
						// Remove the trait from random spawn list
						traitTypesList.Remove( TraitType.RandomizeKit );
						// Apply the randomisation effects to weapon and talent
						CharacterCreator_ApplyRandomizeKitTrait( characterData );
					} else {
						// Override for contrarian on left heir, if it is not in the guaranteed spawns
						if( traitTypes[0] != TraitType.RandomizeKit && traitTypes[1] != TraitType.RandomizeKit ) {
			...
			// Method to access private method CharacterCreator.ApplyRandomizeKitTrait
			private static void CharacterCreator_ApplyRandomizeKitTrait( CharacterData charData ) {
				Traverse.Create( typeof( CharacterCreator ) ).Method( "ApplyRandomizeKitTrait", new Type[] { typeof( CharacterData ) } ).GetValue( new object[] { charData } );
			}
		...
		internal static class LineageWindowController_CreateRandomCharacters_Patch {
			internal static IEnumerable<CodeInstruction> Transpiler( IEnumerable<CodeInstruction> instructions ) {
				...
				if( WobSettings.Get( "SoulShop", "ClassLockContrarian", false ) ) {
					...
					WobPlugin.Log( "Patch disabled in config" );
					return instructions;
                }
			}
		}
